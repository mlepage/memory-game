...
void MemoryGame::finalize()
{
    SAFE_RELEASE(_scene);
}

void MemoryGame::update(float elapsedTime)
{
    // Rotate model
    _scene->findNode("box")->rotateY(MATH_DEG_TO_RAD((float)elapsedTime / 1000.0f * 180.0f));
}

void MemoryGame::render(float elapsedTime)
{
    // Clear the color and depth buffers
    clear(CLEAR_COLOR_DEPTH, Vector4::zero(), 1.0f, 0);

    // Visit all the nodes in the scene for drawing
    _scene->visit(this, &MemoryGame::drawScene);
}

bool MemoryGame::drawScene(Node* node)
{
...
void MemoryGame::keyEvent(Keyboard::KeyEvent evt, int key)
{
...
void MemoryGame::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    switch (evt)
    {
